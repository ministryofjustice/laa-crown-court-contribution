...
public interface CorrespondenceRuleRepository extends JpaRepository<CorrespondenceRule, Integer> {
    ...
    @Query(
            value = "SELECT R.CALC_CONTRIBS AS calcContribs, R.UPLIFT_COTE_ID AS upliftCoteId, "
                    ...
                    + "AND (INIT_RESULT  = 'ANY' OR INIT_RESULT  = :initResult );",
            nativeQuery = true)
    Optional<CorrespondenceRuleAndTemplateInfo> getCoteInfo(
            String meansResult, String iojResult, String magsOutcome, String ccOutcome, String initResult);
}
