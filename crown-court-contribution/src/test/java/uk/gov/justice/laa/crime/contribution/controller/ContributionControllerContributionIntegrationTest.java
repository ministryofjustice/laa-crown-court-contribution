...
class ContributionControllerContributionIntegrationTest {
    ...
    private static final String CHECK_CONTRIBUTION_RULE_ENDPOINT_URL =
            "/api/internal/v1/contribution/check-contribution-rule";
    ...
    void stubForOAuth() throws JsonProcessingException {
        ...
        wiremock.stubFor(post("/oauth2/token").willReturn(WireMock.okJson(objectMapper.writeValueAsString(token))));
    }

    private void setupAppealStubbing(
            ApiMaatCalculateContributionRequest appealContributionRequest, Contribution contribution)
        ...
        wiremock.stubFor(get(urlPathMatching(findContributionUrl))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(MediaType.APPLICATION_JSON))
                        .withBody(objectMapper.writeValueAsString(List.of(contribution)))));
    }
    ...
    void givenAppealCaseWithSameContributions_whenCalculateContributionIsInvoked_thenOkResponse() throws Exception {
        ...
        setupAppealStubbing(
                appealContributionRequest,
                Contribution.builder().upfrontContributions(BigDecimal.ZERO).build());
    ...
    void givenAppealCaseWithDiffContributions_whenCalculateContributionIsInvoked_thenOkResponse() throws Exception {
        ...
        setupAppealStubbing(
                appealContributionRequest,
                Contribution.builder().upfrontContributions(BigDecimal.TEN).build());
    ...
    void givenMaatApiException_whenCalculateContributionIsInvoked_thenInternalServerErrorResponse() throws Exception {
        ...
        var repOrderUrl = "/rep-orders/" + appealContributionRequest.getRepId();
        ErrorDTO errorDTO =
                ErrorDTO.builder().code(HttpStatus.NOT_IMPLEMENTED.name()).build();
        wiremock.stubFor(get(urlEqualTo(repOrderUrl))
    ...
    void givenApiMaatCalculateContributionRequest_whenGetContributionSummariesIsInvoked_thenOkResponse()
            throws Exception {
        List<ContributionsSummaryDTO> contributionsSummaryDTOList =
